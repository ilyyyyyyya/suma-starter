...
- **Token-usage chart history**: the chart reads local Claude Code logs from `~/.claude/projects` and Codex logs from `~/.codex/sessions` plus `~/.codex/archived_sessions`. Claude Code auto-deletes transcripts older than `cleanupPeriodDays` (default 30), so the "All" range only goes back as far as the files still on disk. If the user wants the chart to accumulate real long-term history, add `"cleanupPeriodDays": 180` (or higher) to `~/.claude/settings.json`. It only affects data captured from then on; already-deleted history can't be recovered. If neither tool has local logs, the widget shows a quiet empty state.
- **Token cost is not a bill**: Suma calculates an API-rate equivalent from public token prices. It cannot infer what a Claude or Codex subscription actually cost the user. Unknown model labels show `n/a` instead of a fake zero.
- **Keep generated output private**: `dashboard.html` contains the user's rendered notes and aggregate token totals. The starter's `.gitignore` excludes it. Never force-add or publish that file.
