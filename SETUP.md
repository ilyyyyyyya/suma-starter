...
- **Token-usage chart history**: the chart at the bottom of the Dashboard reads Claude Code's session transcripts from `~/.claude/projects`. Claude Code auto-deletes transcripts older than `cleanupPeriodDays` (default 30), so the "All" range only goes back as far as the files still on disk. If the user wants the chart to accumulate real long-term history, add `"cleanupPeriodDays": 180` (or higher) to `~/.claude/settings.json`. It only affects data captured from then on — already-deleted history can't be recovered. If the user doesn't use Claude Code, the widget shows a quiet empty state.
